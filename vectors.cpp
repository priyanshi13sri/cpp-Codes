...
void expalinVectors(){
    ...
    vector<int>v;  // this line creates a empty container
    v.push_back(1);
    v.emplace_back(2);  //it dynamically increases its size ,it is faster than push_back.

    vector<pair<int,int>vec;

    v.push_back({1,2});
    v.emplace_back(1,2);
    vector<int> v(5,100); // it is a container in which we have 5 instances of 100 
    vector<int> v(5); 
    vector<int> v1(5,20);
    vector<int> v2(v1); // v2 is another container but a copy of v1
    }
    */

// A vector is a container which is dynamic in nature, we can always increase the size of a vector whenever we want to
